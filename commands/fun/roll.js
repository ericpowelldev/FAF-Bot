  ...
  run: async (client, message, args) => {
    if (args[0] && args[0].includes(`d`)) {
      ...
      return message.reply(
        `rolled ${cb}${amount}d${sides}${
          resMod !== 0 ? (resMod > 0 ? "+" + resMod : resMod) : ``
        }${cb} and got ${cb}${total}${cb}${args[0] === `4d20+69` ? ` ( ͡° ͜ʖ ͡°)` : ``}`
      );
    } else {
      const result = rdmInt(1, 20);
      return message.reply(
        `rolled ${cb}1d20${cb} and got ${cb}${result}${cb}${
          result === 20 ? ` --> Crit Success` : result === 1 ? ` --> Crit Fail` : ``
        }`
      );
    }
  },
