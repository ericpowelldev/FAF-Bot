...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    const embed = new RichEmbed()
      ...
      .addField(
        `User Info`,
        stripIndents`**Username:**\xa0\xa0\xa0${member.user.username}
                    **Tag:**\xa0\xa0\xa0#${member.user.tag.split(`#`)[1]}
                    **ID:**\xa0\xa0\xa0${member.user.id}
                    **Created:**\xa0\xa0\xa0${created}`,
        true
      )

      .addField(
        `Member Info`,
        stripIndents`**Nickname:**\xa0\xa0\xa0${member.displayName}
                    **Roles:**\xa0\xa0\xa0${roles}
                    **Joined:**\xa0\xa0\xa0${joined}`,
        true
      )

      .setTimestamp();

    if (member.user.presence.game) embed.addField(`Playing`, `**${member.presence.game.name}**`);
